...
def test_resolver_peso_prefiere_shopify_y_cae_al_titulo():
    assert PR.resolver_peso("Algo 300 g", 280) == (280.0, "shopify")
    assert PR.resolver_peso("Algo 300 g", 0) == (300.0, "titulo")
    assert PR.resolver_peso("Algo sin formato", 0) == (None, "desconocido")
...
# --------------------------------------------------------------------------- #
# rebuild.py — la base debe ser un derivado exacto de los crudos
# --------------------------------------------------------------------------- #
...
def test_rebuild_reproduce_la_base(tmp_path, monkeypatch):
    ...
    catalogos = {
        ...
        "2026-08-15": [producto(1, "Peras", "1.40", "3.00")],
    }
...
def test_rebuild_es_idempotente(tmp_path, monkeypatch):
    import rebuild               # noqa: PLC0415
    raw = tmp_path / "raw"; raw.mkdir()
    with gzip.open(raw / "products_2026-08-14.json.gz", "wt", encoding="utf-8") as fh:
        _json.dump([producto(1, "Peras", "1.50", "3.00")], fh)
    monkeypatch.setattr(rebuild, "RAW_DIR", raw)
    destino = tmp_path / "d.sqlite"
    rebuild.reconstruir(destino, verbose=False)
    rebuild.reconstruir(destino, verbose=False)
    conn = sqlite3.connect(destino)
    assert conn.execute("SELECT COUNT(*) FROM observations").fetchone()[0] == 1
